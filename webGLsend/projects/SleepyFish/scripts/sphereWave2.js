...
function Sphere(radius, segments, x, y, z) {
	this.sphere = new THREE.Mesh(new THREE.SphereGeometry(radius, segments, segments), new THREE.MeshLambertMaterial({shading: THREE.FlatShading, color: 0xff0000}));
...
function canvasWidth() {
    return document.styleSheets.namedItem("canvas").width;
}

function canvasHeight() {
    return document.styleSheets.namedItem("canvas").height;
}
...
function init() {
    ...
    container = document.getElementById("glWrapper");
	...
	info.innerHTML = 'Sphere Wave with Trackball';
